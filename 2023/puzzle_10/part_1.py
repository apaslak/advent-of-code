...
def find_farthest_point(map):
    ...
    while queue:
        ...
        for new_x, new_y in possible_moves:
            ...
            log(f"added {(new_x, new_y)} to queue, steps {steps_to_next_move}")
...
def test_examples():
    map = parse_input_file(r"test_input1.txt")
    answer = find_farthest_point(map)

    assert answer == 4

    map = parse_input_file(r"test_input2.txt")
    answer = find_farthest_point(map)
