...
def calculate_distance_from_origin(central_port_x, central_port_y, line, point):
    distance = 0
    last_point_x = central_port_x
    last_point_y = central_port_y
    for y, x in line:
        distance += abs(last_point_x - x) + abs(last_point_y - y)
        if point == (y, x):
            break
        last_point_x = x
        last_point_y = y
...
def calculate_shortest_distance(central_port_x,
                                central_port_y,
                                line1_points,
                                line2_points,
                                intersection_points):
    ...
    for intersection_point in intersection_points:
        distance = calculate_distance_from_origin(central_port_x,
                                                  central_port_y,
                                                  line1_points,
                                                  intersection_point)
        distance += calculate_distance_from_origin(central_port_x,
...
def test_examples():
    ...
    assert calculate_shortest_distance(central_port_x,
                                       central_port_y,
                                       line1_points,
                                       line2_points,
...
def puzzle3():
    ...
    print(f'calculate_manhattan_distance: {result}')
    result = calculate_shortest_distance(central_port_x,
                                         central_port_y,
                                         line1_points,
                                         line2_points,
                                         intersection_points)
    print(f'calculate_shortest_distance: {result}')
